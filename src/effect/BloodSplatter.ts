...
export class BloodSplatter
{
    ...
    {
        for (var i:number = 0; i < this.num; i += 1)
        {
            //创建血迹
            var blood:egret.Bitmap = new egret.Bitmap(this.bloodTextrue);
    ...
    public clear():void
    {
        var count:number = this.bloodList.length;
        for(var i:number = count - 1; i >= 0; --i)
        {
            var blood:egret.Bitmap = this.bloodList[i];
            if(blood &&
               blood.parent)
