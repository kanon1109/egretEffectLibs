...
export class SlotsEffect
{
	...
	private timerHandler(event:egret.TimerEvent):void
	{
		...
		var length:number = this.funList.length;
		for (var i:number = 0; i < length; i += 1)
		{
			var thisObj:any = this.thisList[i];
	...
	{
		if(!this.funList) return;
		var index:number = this.funList.indexOf(fun);
		if (index != -1)
		{
	...
	public destroy():void
	{
		...
		if (this.thisList)
		{
			var length:number = this.thisList.length;
			for (var i:number = length - 1; i >= 0; i -= 1) {
