...
export class ResidueShadowEffect
{
    ...
    public addGoods(dspObj:egret.DisplayObject):void
    {
        ...
        var index:number = this.goodsList.indexOf(dspObj);
        if(index == -1)
        {
            this.goodsList.push(dspObj);
            var obj:Object = {"x":dspObj.x, "y":dspObj.y};
    ...
    {
        if(!this.goodsList) return;
        for(var i:number = 0; i < this.goodsList.length; ++i)
        {
            var goodsObj:egret.DisplayObject = this.goodsList[i];
            var obj:Object = this.goodsDataList[i];
            var x:number = obj["x"];
            var y:number = obj["y"];
            var dis:number = this.mathDistance(x, y, goodsObj.x, goodsObj.y);
    ...
    private createShadow(obj:egret.DisplayObject):void
    {
        ...
        {
            var mc:egret.MovieClip = <egret.MovieClip>obj;
    ...
    {
        if(!this.goodsList) return;
        var length:number = this.goodsList.length;
